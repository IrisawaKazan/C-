...
HRESULT CManager::Init(HINSTANCE nInstance, HWND hWnd, BOOL bWindow)
{
	...
	// オブジェクトの生成
	m_pBackground = CBackground::Create(D3DXVECTOR3(SCREEN_WIDTH, SCREEN_HEIGHT, 0.0f)); // 背景
	...
	m_pPlayer = CPlayer::Create(D3DXVECTOR3(SCREEN_WIDTH / 2.0f, SCREEN_HEIGHT / 2.0f, 0.0f)); // プレイヤー(ランニングマン正面)
}
