...
// コンストラクタ
//----------------------------------------
CSpaceshipX::CSpaceshipX(int nPriority) : CObject(nPriority)
{
	m_pTexture = nullptr;
	m_pMesh = nullptr;
	m_pBuffMat = nullptr;
...
// 生成処理
//----------------------------------------
CSpaceshipX* CSpaceshipX::Create(D3DXVECTOR3 pos)
{
	CSpaceshipX* pSpaceshipX;

	// オブジェクトXの生成
	pSpaceshipX = new CSpaceshipX;

	// 初期化処理
	pSpaceshipX->Init();

	// 位置の設定
	pSpaceshipX->SetPosition(pos);
...
HRESULT CSpaceshipX::Init(void)
{
	...
	// Xファイルの読み込み
	D3DXLoadMeshFromX("data\\MODEL\\spaceship.x",
		D3DXMESH_SYSTEMMEM,
		pDevice,
		NULL,
		&m_pBuffMat,
...
// 位置の設定処理
//----------------------------------------
void CSpaceshipX::SetPosition(D3DXVECTOR3 pos)
{
	m_pos = pos;
}
...
// 位置の取得処理
//----------------------------------------
D3DXVECTOR3 CSpaceshipX::GetPos(void)
{
	return m_pos;
}