...
// トゲトゲの取得処理
//----------------------------------------
CDebrisX* CDebrisX::GetDebrisX(void)
{
...
{
	// 左右のめり込み判定
	if (CObjectX::GetObjectX()->GetPos().z + CObjectX::GetObjectX()->GetSize().z / 2.0f > m_pos.z + m_vtxMax.z &&
		CObjectX::GetObjectX()->GetPos().z + CObjectX::GetObjectX()->GetSize().z / 2.0f < m_pos.z - m_vtxMin.z * 2.0f)
	{
		// 左から右へ
		if (CObjectX::GetObjectX()->GetPosOld().x + CObjectX::GetObjectX()->GetSize().x / 2.0f > m_pos.x + m_vtxMin.x &&
			CObjectX::GetObjectX()->GetPos().x + CObjectX::GetObjectX()->GetSize().x / 2.0f < m_pos.x - m_vtxMin.x)
		{
			CObjectX::GetObjectX()->Uninit();
		}
		// 右から左へ
		if (CObjectX::GetObjectX()->GetPosOld().x - CObjectX::GetObjectX()->GetSize().x / 2.0f < m_pos.x - m_vtxMax.x &&
			CObjectX::GetObjectX()->GetPos().x - CObjectX::GetObjectX()->GetSize().x / 2.0f > m_pos.x + m_vtxMax.x)
		{
			CObjectX::GetObjectX()->Uninit();
		}
	}
