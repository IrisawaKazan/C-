...
// コンストラクタ
//----------------------------------------
CZomchinX::CZomchinX(int nPriority) : CObject(nPriority)
{
	m_pTexture = nullptr;
	m_pMesh = nullptr;
	m_pBuffMat = nullptr;
...
// 生成処理
//----------------------------------------
CZomchinX* CZomchinX::Create(D3DXVECTOR3 pos, ZOMCHIN type)
{
	CZomchinX* pZomchinX;
	...
	{
		// オブジェクトXの生成
		pZomchinX = new CZomchinX;

		// 初期化処理
		pZomchinX->Init(pos);

		// タイプの設定
		pZomchinX->SetType(type);

		// 位置の設定
		pZomchinX->SetPosition(pos);
...
// 初期化処理
//----------------------------------------
HRESULT CZomchinX::Init(D3DXVECTOR3 pos)
{
	// 種類の設定処理
	CObject::SetType(TYPE_ZOMCHIN);
	...
	// Xファイルの読み込み
	D3DXLoadMeshFromX("data\\MODEL\\zomchin000.x",
		D3DXMESH_SYSTEMMEM,
		pDevice,
		NULL,
		&m_pBuffMat,
...
void CZomchinX::Update(void)
{
	...
	//m_rot = D3DXVECTOR3(0.0f, m_fRotation / 175.0f/* 回る速度 */, 0.0f);
...
// 位置の設定処理
//----------------------------------------
void CZomchinX::SetPosition(D3DXVECTOR3 pos)
{
	m_pos = pos;
}
...
// 向きの設定処理
//----------------------------------------
void CZomchinX::SetRotation(D3DXVECTOR3 rot)
{
	m_rot = rot;
}
...
// 位置の取得処理
//----------------------------------------
D3DXVECTOR3 CZomchinX::GetPos(void)
{
	return m_pos;
}
...
// 向きの取得処理
//----------------------------------------
D3DXVECTOR3 CZomchinX::GetRot(void)
{
	return m_rot;
}
...
// タイプの設定処理
//----------------------------------------
void CZomchinX::SetType(ZOMCHIN type)
{
	m_type = type;
}
...
// タイプの取得処理
//----------------------------------------
CZomchinX::ZOMCHIN CZomchinX::GetType(void)
{
	return m_type;
}
