...
void CRenderer::Draw(void)
{
	...
	//m_pD3DDevice->GetRenderTarget(0, &m_apRenderMT[0]);
	//
	//// テクスチャ[0]のクリア
	...
	if (SUCCEEDED(m_pD3DDevice->BeginScene()))
	{
		...
		{
			// ObjectXのpos.zをデバッグ表示
			CDebugProc::Print("\nPlayerPosZ: %.0f", pObjectX->GetPos().z);

			CDebugProc::Draw();

			// ObjectXのm_nDeadCounterをデバッグ表示
			CDebugProc::Print("\n\nDeadCounter: %d(600で死亡)", pObjectX->GetDeadCounter());
...
//
//	// レンダリングターゲットを生成したテクスチャに設定
//	m_pD3DDevice->GetRenderTarget(0, &m_pZBuffMT);
//
//	// Zバッファを生成したZバッファに設定
//	m_pD3DDevice->SetDepthStencilSurface(m_pZBuffMT);
//
//	// テクスチャレンダリング用もビューポートを設定
...
//
////----------------------------------------
//// レンダリングターゲット用テクスチャの取得処理
////----------------------------------------
//LPDIRECT3DTEXTURE9 CRenderer::GetTextureMT(void)
//{
//	for (int nCnt = 0; nCnt < 2; nCnt++)
//	{
//		return m_apTextureMT[nCnt];
//	}
//}