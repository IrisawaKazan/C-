...
void CManager::Update(void)
{
	...
	if (pInputKeyboard->GetTrigger(DIK_TAB) || pInputKeyboard->GetTrigger(DIK_P) || pInputJoypad->GetTrigger(pInputJoypad->JOYKEY_START) == true)
	{// ポーズキーが押された
	...
	{// ポーズ中でなければ
		...
		// アイテム→
		if (m_nDebrisSpawn >= rand() / MAX_STARDUST_SPAWN && m_nFrameCounter[8] >= NUM_FRAME_CNT && m_pObjectX->GetSigns() == false && m_pObjectX->GetPowerUp() == false)
		{
			CStardustX::Create(D3DXVECTOR3(-STARDUST_000_POS_X, 0.0f, fPosZ), CStardustX::STARDUST_000_A);

			m_nDebrisSpawn = 0;
			m_nFrameCounter[8] = 0;
		}
		if (m_nDebrisSpawn >= rand() / MAX_STARDUST_SPAWN && m_nFrameCounter[9] >= NUM_FRAME_CNT && m_pObjectX->GetSigns() == false && m_pObjectX->GetPowerUp() == false)
		{
			CStardustX::Create(D3DXVECTOR3(-STARDUST_000_POS_X, 0.0f, -fPosZ), CStardustX::STARDUST_000_A);
		...
		// ←アイテム
		if (m_nDebrisSpawn >= rand() / MAX_STARDUST_SPAWN && m_nFrameCounter[10] >= NUM_FRAME_CNT && m_pObjectX->GetSigns() == false && m_pObjectX->GetPowerUp() == false)
		{
			CStardustX::Create(D3DXVECTOR3(STARDUST_000_POS_X, 0.0f, fPosZ), CStardustX::STARDUST_000_B);

			m_nDebrisSpawn = 0;
			m_nFrameCounter[10] = 0;
		}
		if (m_nDebrisSpawn >= rand() / MAX_STARDUST_SPAWN && m_nFrameCounter[11] >= NUM_FRAME_CNT && m_pObjectX->GetSigns() == false && m_pObjectX->GetPowerUp() == false)
		{
			CStardustX::Create(D3DXVECTOR3(STARDUST_000_POS_X, 0.0f, -fPosZ), CStardustX::STARDUST_000_B);
		...
		// 10コイン→
		if (m_nDebrisSpawn >= rand() / MAX_COIN10_SPAWN && m_nFrameCounter[12] >= NUM_FRAME_CNT)
		{
			CCoin10X::Create(D3DXVECTOR3(-COIN10_000_POS_X, 0.0f, fPosZ), CCoin10X::COIN10_000_A);

			m_nDebrisSpawn = 0;
			m_nFrameCounter[12] = 0;
		}
		if (m_nDebrisSpawn >= rand() / MAX_COIN10_SPAWN && m_nFrameCounter[13] >= NUM_FRAME_CNT)
		{
			CCoin10X::Create(D3DXVECTOR3(-COIN10_000_POS_X, 0.0f, -fPosZ), CCoin10X::COIN10_000_A);
		...
		// ←10コイン
		if (m_nDebrisSpawn >= rand() / MAX_COIN10_SPAWN && m_nFrameCounter[14] >= NUM_FRAME_CNT)
		{
			CCoin10X::Create(D3DXVECTOR3(COIN10_000_POS_X, 0.0f, fPosZ), CCoin10X::COIN10_000_B);

			m_nDebrisSpawn = 0;
			m_nFrameCounter[14] = 0;
		}
		if (m_nDebrisSpawn >= rand() / MAX_COIN10_SPAWN && m_nFrameCounter[15] >= NUM_FRAME_CNT)
		{
			CCoin10X::Create(D3DXVECTOR3(COIN10_000_POS_X, 0.0f, -fPosZ), CCoin10X::COIN10_000_B);
	...
	// パッドのバックキーでウィンドウ終了
	if (pInputJoypad->GetTrigger(pInputJoypad->JOYKEY_BACK) == true)
	{
		// ウィンドウを閉じる
		PostQuitMessage(0);
	}
