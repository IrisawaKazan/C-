...
// 生成処理
//----------------------------------------
CTutorialOBJ* CTutorialOBJ::Create(D3DXVECTOR3 pos)
{
	CTutorialOBJ* pTutorialOBJ;
...
HRESULT CTutorialOBJ::Init(void)
{
	...
	m_pos = D3DXVECTOR3(0.0f, 0.0f, 0.0f);
	m_rot = D3DXVECTOR3(D3DX_PI / -2.0f, 0.0f, 0.0f);
	//m_rot = D3DXVECTOR3(D3DX_PI / -7.5f, 0.0f, 0.0f);

	// Xファイルの読み込み
	D3DXLoadMeshFromX("data\\MODEL\\tutorialOBJ.x",
		D3DXMESH_SYSTEMMEM,
		pDevice,
		NULL,
		&m_pBuffMat,
...
void CTutorialOBJ::Update(void)
{
	...
	if (m_pos.z >= 75.0f)
	{
		...
		if (m_nSECounter == 15)
		{
			pSound->PlaySoundA(CSound::SOUND_LABEL_ERROR_SE);
...
// 位置の設定処理
//----------------------------------------
void CTutorialOBJ::SetPosition(D3DXVECTOR3 pos)
{
	m_pos = pos;
}
...
// 位置の取得処理
//----------------------------------------
D3DXVECTOR3 CTutorialOBJ::GetPos(void)
{
	return m_pos;
}