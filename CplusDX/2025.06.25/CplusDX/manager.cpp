...
//CSphereX* CManager::m_pSphere = nullptr;
...
HRESULT CManager::Init(HINSTANCE nInstance, HWND hWnd)
{
	...
	//CSphereX::Create(D3DXVECTOR3(0.0f, 0.0f, 0.0f));

	// オブジェクトビルボード(木)の生成(初期配置)
	//CObjectBillboard::Create(D3DXVECTOR3(75.0f, 50.0f, 0.0f)/*, 50.0f, 50.0f*/);

	// 宇宙ゴミの生成(初期配置)
	//CDebrisX::Create(D3DXVECTOR3(0.0f, 0.0f, 100.0f), CDebrisX::DEBRIS_000_A);
...
void CManager::Update(void)
{
	...
	{// ポーズ中でなければ
		...
		// トゲトゲ→
		if (m_nDebrisSpawn >= rand() / MAX_SPAWN && m_nFrameCounter >= NUM_FRAME_CNT)
		{
			CDebrisX::Create(D3DXVECTOR3(-300.0f, 0.0f, fPosZ), CDebrisX::DEBRIS_000_A);
		...
		if (m_nDebrisSpawn >= rand() / MAX_SPAWN && m_nFrameCounter >= NUM_FRAME_CNT)
		{
			CDebrisX::Create(D3DXVECTOR3(300.0f, 0.0f, fPosZ), CDebrisX::DEBRIS_000_B);
...
//// スフィアXの取得処理
////----------------------------------------
//CSphereX* CManager::GetSphereX(void)
//{
//	return m_pSphere;
//}